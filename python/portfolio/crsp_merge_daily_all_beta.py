...
crsp_d = pyreadr.read_r('data/rdata/stoxda_around2000.Rdata') #filtered CRSP data (received from Pedro)
#print(crsp_d.keys())
crsp_d_df = crsp_d['stoxda_around2000'] # converting the R object to a pandas dataframe
mkt = pd.read_csv('data/csv/F-F_Research_Data_Factors_daily.CSV')

mkt_rf = mkt[['date', 'Mkt-RF']]
mkt_rf = (mkt_rf
        .assign(date = pd.to_datetime(mkt_rf['date'], format='%Y%m%d'))
...
crsp_d_pre_merge = (pd.merge(crsp_d_pre_merge, mkt_rf, how = 'left', on = 'date'))
...
crsp_all = (crsp_d_pre_merge_no_dup
...
ccm_all = (pd.merge(crsp_all, compust_pre_merge, how = 'left', on = 'GVKEY_rdq'))
#ccm_all.head(50)
...
# Save dataframe
ccm_all.to_feather('data/feather/ccm_all.feather')
