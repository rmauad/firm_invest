...
source('code/intan_mp/R/prep_dta/ff_ind.R')
...
data_intan_q_new <- data_intan_q %>% select(GVKEY, cusip, year, year_q, sic, atq, ceqq, dlcq, dlttq, 
                                            ...
                                            saleq, ibq, dpq, 
                                            ppentq, CPI, RGDP, Ind_prod, dln_RGDP, d_Ind_prod, dltisy, emp, capxy) %>%
  ...
  #mutate(log_inflation = log_cpi - dplyr::lag(log_cpi)) %>%
...
# SAVING ####
# save(db_reg, file = "data/rdata/db_reg_visual.RData")
# write.dta(db_reg_comp, "db_reg_20220806.dta")
#write.dta(db_reg_1989, "data/dta/db_reg_1989_only.dta") #only firms in 1989
write.dta(db_reg, "data/dta/db_reg_q_cf_old_controls.dta") #all firms
