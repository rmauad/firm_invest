...
comp_sel_q <- select(comp_fundq, GVKEY, cusip, datadate, rdq, fqtr, sic, xsgaq, intanq, atq, ceqq, dlcq, dlttq, ppegtq, revtq, cheq, ltq, cshoq, prccq, saleq, ibq, dpq, ppentq, capxy, pstkq, invtq, xintq, dltisy, txdbq, xrdq)
...
# Export GVKEY and state information to .dta (to generate a dummy in Stata)
load('data/rdata/comp_fundq.Rdata')
state <- comp_fundq %>%
  select(GVKEY, state) %>%
  filter(state != "")

state <- state[!duplicated(state$GVKEY),]
state <- state %>%
  mutate(GVKEY = as.numeric(GVKEY))
save(state, file = "data/rdata/state.rdata") 
write.dta(state, "data/dta/state.dta") 

# Selecting relevant variables from Compustat
cap_struct <- comp_fundq %>%
  select(GVKEY, datadate, dltisy, dltry, dlcchy, sstky) %>%
  filter(year(datadate) >= 1990) %>%
  na.omit()


write.csv(cap_struct, "data/csv/cap_struct.csv")
