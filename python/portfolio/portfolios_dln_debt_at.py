...
smallest_non_zero = df.loc[df['d_ln_debt_at'] > 0, 'd_ln_debt_at'].min()
print(smallest_non_zero)
epsilon = np.random.uniform(0, smallest_non_zero * 0.1, size=len(df))

smallest_non_zero_lev = df.loc[df['debt_at'] > 0, 'debt_at'].min()
...
df['d_ln_debt_at_adj'] = df['d_ln_debt_at']
df['debt_at_adj'] = df['debt_at']
df.loc[df['d_ln_debt_at'] == 0, 'd_ln_debt_at_adj'] = epsilon[df['d_ln_debt_at'] == 0]
df.loc[df['debt_at'] == 0, 'debt_at_adj'] = epsilon_lev[df['debt_at'] == 0]

# df.head(50)

df['qui_d_ln_debt_at'] = df.groupby('year_month')['d_ln_debt_at_adj'].transform(
    lambda x: pd.qcut(x, 5, labels=[1, 2, 3, 4, 5])
)

df['qui_debt_at'] = df.groupby('year_month')['debt_at_adj'].transform(
    lambda x: pd.qcut(x, 5, labels=[1, 2, 3, 4, 5])
)

grouped = df.groupby(['year_month', 'qui_debt_at', 'qui_d_ln_debt_at'])
...
overall_avg_returns = monthly_avg_returns.groupby(['qui_debt_at', 'qui_d_ln_debt_at'])['RET'].mean().reset_index()

# pivot_df = overall_avg_returns.pivot(index='qui_d_debt_at', columns='qua_intan', values='ret_3mo_lead1')
pivot_df = overall_avg_returns.pivot(index='qui_d_ln_debt_at', columns='qui_debt_at', values='RET')
