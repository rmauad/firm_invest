...
foreach bvdidnumber of local firm_list {
	...
	by bvdidnumber: replace org_cap = (1 - delta_init) * org_cap[_n- 1] + sga if _n > 1
}

gen intan_tan = org_cap/netpropertyplantequipment
collapse (sum) org_cap sga netsales operatingplebit netpropertyplantequipment totalassets intangibles, by(year)
gen intan_cap = org_cap/(org_cap+netpropertyplantequipment)
gen intan_tan = org_cap/netpropertyplantequipment
gen intan_sales = org_cap/netsales
gen intan_at = org_cap/totalassets

collapse (sum) netpropertyplantequipment totalassets intangibles, by(year)

gen intan_cap = intangibles/(intangibles+netpropertyplantequipment)
gen intan_tan = intangibles/netpropertyplantequipment
gen intan_sales = intangibles/netsales
gen intan_at = intangibles/totalassets

line intan_tan year

save /homes/nber/mauadr/bulk/orbis.work/intan_mp/data/dta/intan_incomp2_30.dta, replace
...
save /homes/nber/mauadr/bulk/orbis.work/intan_mp/data/dta/intan_comp_2yt_intan_at.dta, replace
// Merging databases
merge m:1 year using /homes/nber/mauadr/bulk/orbis.work/intan_mp/data/dta/db_reg_y_orbis_2yt.dta, keep(3) nogen //only keep matched observations
//drop if year == year[_n-1] //in case there are repeated time values within the panel
save /homes/nber/mauadr/bulk/orbis.work/intan_mp/data/dta/db_reg_orbis_2yt_intan_at.dta, replace

// Selecting non-publicly traded firms
use bvdidnumber stockexchangeslisted using exchanges.dta
duplicates drop bvdidnumber, force
merge 1:m bvdidnumber using /homes/nber/mauadr/bulk/orbis.work/intan_mp/data/dta/db_reg_orbis_2yt_intan_at.dta, keep(2) //keep (2) preserves the observations from b_reg_orbis.dta that did not have an equivalent in exchanges.dta
save /homes/nber/mauadr/bulk/orbis.work/intan_mp/data/dta/db_reg_orbis_npt_2yt_intan_at.dta, replace
//save /homes/nber/mauadr/bulk/orbis.work/intan_mp/data/dta/db_reg_orbis_pt_2yt_intan_at.dta, replace
//save /homes/nber/mauadr/bulk/orbis.work/intan_mp/data/dta/intan_orig_pt.dta, replace
