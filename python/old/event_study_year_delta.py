...
percentile_98 = df_year_tang['d_debt_at_adj'].quantile(0.98)
df_year_tang_filtered = df_year_tang[(df_year_tang['year'] >= 1995) & (df_year_tang['year'] <= 1999) & (df_year_tang['category'] != 'other') & (df_year_tang['d_debt_at_adj'] < percentile_98)]
...
df_year_intan = df_intan.groupby(['GVKEY', 'year']).agg({'d_debt_at_adj': 'sum', 'debt_at': 'mean', 'category': 'first', 'group': 'first'}).reset_index()
percentile_98 = df_year_intan['d_debt_at_adj'].quantile(0.98)
df_year_intan_filtered = df_year_intan[(df_year_intan['year'] >= 1995) & (df_year_intan['year'] <= 1999) & (df_year_intan['category'] != 'other') & (df_year_intan['d_debt_at_adj'] < percentile_98)]
...
sns.violinplot(x='year', y='d_debt_at_adj', hue = 'group', data=df_year_tang_filtered, split=True)
...
sns.violinplot(x='year', y='d_debt_at_adj', hue = 'group', data=df_year_intan_filtered, split=True)
plt.title('Debt Issuance by Firm type and Year')
plt.xlabel('Years')
plt.ylabel('Debt Issuance (million $)')
...
percentile_98 = df_tang['d_debt_at_adj'].quantile(0.98)
df_tang_trim = df_tang[df_tang['d_debt_at_adj'] < percentile_98]

percentile_98 = df_intan['d_debt_at_adj'].quantile(0.98)
df_intan_trim = df_intan[df_intan['d_debt_at_adj'] < percentile_98]
...
y = df_tang_treated['d_debt_at_adj']
...
y_intan = df_intan_treated['d_debt_at_adj']
