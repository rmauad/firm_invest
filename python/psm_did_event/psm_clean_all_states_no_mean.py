...
db_did_clean = db_did_na_clean.dropna(subset=metrics).copy() # .copy() guarantees that a new independent dataframe is created
#X = db_did_clean[metrics_mean]  # the financial metrics
X = db_did_clean[metrics]  # the financial metrics
...
smds = {covariate: calculate_smd(treated_tx_la, control_tx_la, covariate) 
        for covariate in ['dln_inv', 'capxy', 'atq', 'debt_at']}
...
sorted_columns = sorted(matched_db.columns)
