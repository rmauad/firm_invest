...
df_year_tang = df_tang.groupby(['GVKEY', 'year']).agg({'debt_at': 'mean', 'treated': 'first',}).reset_index()
percentile_98 = df_year_tang['debt_at'].quantile(0.98)
df_tang_trim = df_year_tang[df_year_tang['debt_at'] < percentile_98]

sorted_columns = sorted(df_tang_trim.columns)
print(sorted_columns)
# Create a dummy 'post' for periods after 1997:Q1
df_tang_trim['post'] = (df_tang_trim['year'] >= 1997).astype(int)
df_tang_trim['treatment_post'] = df_tang_trim['treated'] * df_tang_trim['post']
df_intan['post'] = (df_intan['year'] > 1997).astype(int)
df_intan['treatment_post'] = df_intan['treated'] * df_intan['post']

#region DiD for tangible firms
model_tang = smf.ols('debt_at ~ treated + post + treatment_post', data=df_tang_trim).fit()
model_intan = smf.ols('debt_at ~ treated + post + treatment_post', data=df_intan).fit()
