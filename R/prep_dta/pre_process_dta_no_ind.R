...
load("data/rdata/db_reg_2yt_no_ind.rdata") # from prep_data_no_ind.R
load("data/rdata/state.rdata")

db_reg_no_ind <- left_join(db_reg_no_ind, state, by = "GVKEY")
  
db_reg_no_ind <- db_reg_no_ind %>%
  group_by(GVKEY) %>%
...
write.csv(db_reg_no_ind, "data/csv/db_reg_no_ind.csv")
