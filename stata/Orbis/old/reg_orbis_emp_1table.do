...
cap rename (d_treas_2y_lag1) (mpshock)
xtreg dln_emp c.mpshock#c.ter_low ln_assets dln_netsales_lag1 cash_at_lag1 ln_RGDP_lag1 d_ind_prod_lag1 CPI_lag1 if stck_exc_ind == 0,fe cluster(ff_indust)
est store emp_low
xtreg dln_emp c.mpshock#c.ter_mid ln_assets dln_netsales_lag1 cash_at_lag1 ln_RGDP_lag1 d_ind_prod_lag1 CPI_lag1 if stck_exc_ind == 0,fe cluster(ff_indust)
est store emp_mid
xtreg dln_emp c.mpshock#c.ter_high ln_assets dln_netsales_lag1 cash_at_lag1 ln_RGDP_lag1 d_ind_prod_lag1 CPI_lag1 if stck_exc_ind == 0,fe cluster(ff_indust)
est store emp_high

xtreg dln_emp c.mpshock#c.ter_low ln_assets dln_netsales_lag1 cash_at_lag1 ln_RGDP_lag1 d_ind_prod_lag1 CPI_lag1 if stck_exc_ind == 1,fe cluster(ff_indust)
est store emp_low_assets
xtreg dln_emp c.mpshock#c.ter_mid ln_assets dln_netsales_lag1 cash_at_lag1 ln_RGDP_lag1 d_ind_prod_lag1 CPI_lag1 if stck_exc_ind == 1,fe cluster(ff_indust)
est store emp_mid_assets
xtreg dln_emp c.mpshock#c.ter_high ln_assets dln_netsales_lag1 cash_at_lag1 ln_RGDP_lag1 d_ind_prod_lag1 CPI_lag1 if stck_exc_ind == 1,fe cluster(ff_indust)
est store emp_high_assets
cap rename (mpshock) (d_treas_2y_lag1)

esttab emp_low emp_mid emp_high emp_low_assets emp_mid_assets emp_high_assets using /homes/nber/mauadr/bulk/orbis.work/intan_mp/output/orbis_emp.tex, coeflabels(c.mpshock#c.ter_low "Δ 2-year Treasury(t-1) x Tercile low" c.mpshock#c.ter_mid "Δ 2-year Treasury(t-1) x Tercile mid" c.mpshock#c.ter_high "Δ 2-year Treasury(t-1) x Tercile high" c.mpshock#c.ter_low#c.ln_assets "Δ 2-year Treasury(t-1) x Tercile low x Log(assets)" c.mpshock#c.ter_mid#c.ln_assets "Δ 2-year Treasury(t-1) x Tercile mid x Log(assets)" c.mpshock#c.ter_high#c.ln_assets "Δ 2-year Treasury(t-1) x Tercile high x Log(assets)") b(%9.3f) label replace se ar2 stats(N r2, fmt(0 %9.3f)) star(* 0.10 ** 0.05 *** 0.01) drop(_cons dln_netsales_lag1 cash_at_lag1 dln_RGDP_lag1 d_ind_prod_lag1 CPI_lag1)
